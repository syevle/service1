...
```
	C:\00_ANA\JavaEE\WS\docker-java-app-example>docker build --tag=service1 .
...
```
	...
	service1   latest              a0c355a25236        About a minute ago   105MB
...
Spring Boot is set to **8080**. So outside of the Docker Container, we will use the port number **81**, and it is mapped to **8080**
and will reach to the Spring Boot application.

As you run the command, you can open up your browser, you can go to this url [http://localhost:81](http://localhost)
...
```
	...
	PID   USER     TIME  COMMAND
	    1 root      0:08 java -jar service1.jar
...
C:\00_ANA\JavaEE\WS\docker-java-app-example>docker container ls
CONTAINER ID        IMAGE                         COMMAND                  CREATED             STATUS              PORTS                  NAMES
159c3ee8ef39        service1            "java -jar docker-ja…"   16 minutes ago      Up 16 minutes       0.0.0.0:80->8080/tcp   adoring_jackson
...
```
	docker run -p 8081:8080 service1
```
